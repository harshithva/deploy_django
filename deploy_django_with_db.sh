...
sudo apt update && sudo apt install -y python3-pip python3-venv nginx sqlite3
...
if grep -q "DATABASES" "$SETTINGS_FILE"; then
    echo "Configuring SQLite database in $SETTINGS_FILE"
    # Use sed to modify the DATABASES setting to use SQLite
    sed -i "/DATABASES = {/a\    'default': {\n        'ENGINE': 'django.db.backends.sqlite3',\n        'NAME': os.path.join(BASE_DIR, 'db.sqlite3'),\n    }" "$SETTINGS_FILE"
else
    echo "DATABASES not found in $SETTINGS_FILE. Exiting..."
    exit 1
fi
